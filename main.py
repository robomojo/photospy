...
class App:
    ...
    def start(self):
        ...
        if self.args.bool_copyFromCameraToHdd:
            ...
            for file in os.listdir(self.args.camera):
                transitingFile = TransitingFile(os.path.join(self.args.camera, file))
                directory = os.path.join(self.args.hdd, transitingFile.timestring[:8])
        ...
        if self.args.bool_copyFromHddToWebsync:
            ...
            for root, dirs, files in os.walk(self.args.hdd):
                for file in files:
                    ...
                    directory = os.path.join(self.args.websync, YYYYMM)
                    if not (os.path.exists(directory)):
                    ...
                    if not (os.path.exists(newImageFilePath)):
                        ...
                        if self.args.bool_resizeWebsyncImage:
                            print image.resize(self.args.resize)
        ...
        if self.args.bool_syncUpdatesFromHddToWebsync:
            ...
            for root, dirs, files in os.walk(self.args.websync):
                for file in files:
                    if file[-3:] != 'JPG': # only do jpg files
                        continue
                    print(file)
...
class Args:
    def __init__(self):
        ...
        self.camera = self.config.get('folders', 'camera')
        self.hdd = self.config.get('folders', 'hdd')
        self.websync = self.config.get('folders', 'websync')
        ...
        self.resize = self.config.getint('options','resize')
